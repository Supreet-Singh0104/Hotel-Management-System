        ...
        b1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    Conn c = new Conn();
                    // Corrected query based on the customer table structure (room instead of room_number)
                    ResultSet rs1 = c.s.executeQuery("select * from customer where number = '" + s1 + "'");

                    if (rs1.next()) {
                        txt_Room.setText(rs1.getString("room"));  // Fetching 'room' from customer table
                    } else {
                ...
                try {
                    ...
                    if (!roomNumber.isEmpty()) {
                        // Corrected query based on the room table structure (roomnumber instead of room_number)
                        ResultSet rs2 = c.s.executeQuery("select * from room where roomnumber = '" + roomNumber + "'");
                        if (rs2.next()) {
                            txt_Ava.setText(rs2.getString("availability"));
                            txt_Status.setText(rs2.getString("cleaning_ststus"));
                        } else {
...
btnUpdate.addActionListener(new ActionListener() {
    public void actionPerformed(ActionEvent e) {
        try {
            Conn c = new Conn();  // Create a new connection instance
            String roomNumber = txt_Room.getText();
            String cleanStatus = txt_Status.getText();

            if (!roomNumber.isEmpty()) {
                String str = "update room set cleaning_ststus = ? where roomnumber = ?";
                
                // Use the 'c' object from the Conn class (c.c is not needed)
